...
prog def scale_transformation
	...
	SINGHMEThod(integer 1) MONOtonicity(integer 1) monofile(string) timeroff save(string)]
	
	** Initial checks
	if inlist(`type',1,2,7,8)==1 & "`compgroup'"=="" {
		di ""
		di as err "Error:  Variable compgroup should be specified if type is 1, 2, 7 or 8."
		e
		}
	if "`compgroup'"!="" {
	...
	if inlist(`type',1,2,3,4,5,6,7,8)==0 {
		di ""
		di as err "Error: Type should be 1 and 2 for Gap Growth max and min; 3 and 4 for Correlation max and min; 5 and 6 for R-squared max and min; or 7 and 8 for Controls Explanation Index max and min, respectively."
		e
		}
	if inlist(`type',7,8)==1 & ("`controls'"=="" & "`controls1'"=="" & "`controls2'"=="")  {
		di ""
		di as err "Error: At least one control variable needed (i.e. controls1 or controls2) for Controls Explanation max and min"
		e
		}
	...
	if `score1'<0 | (`score1'>1 & `score1'!=.)  {
		di ""
		di as err "Error: Variable `score1' should be scaled to be between 0 and 1"
		e
		}
	if `score2'<0 | (`score2'>1 & `score2'!=.)  {
		di ""
		di as err "Error: Variable `score2' should be scaled to be between 0 and 1"
		e
		}
	if `iterations'<=0 {
		di ""
	...
	** Locals
	local seed = round(uniform()*1000000)
	set seed  `seed'
	...
	* Run loop using random sets of initial parameters
	...
	forv row = 1/`obs' {
		...
		di ""  _newline(1) 	
		di "Optimizing - Iteration `row'/`obs'"  _newline(1) 
...
m
	...
	void function objf(todo, p, sesgroup, irt1, irt2, controls, c1, c2, w, mono_data, type, obj, g, H) {
		...
		mean_s_irt1 = mean(s_irt1,w)
		var_s_irt1 = quadvariance(s_irt1,w)
		s_irt1 = (s_irt1:-mean_s_irt1):/(var_s_irt1^.5)
		
		mean_s_irt2 = mean(s_irt2,w)
		var_s_irt2 = quadvariance(s_irt2,w)
